	...
	Describe("podE2ETest", func() {
		Context("Create backup of pod and restore", func() {
			It("pod with replicas and pods", func() {
				...
				Expect(err).To(BeNil())
				log.Infof(" pod %v in ns %v is deleted\n", name, ns)
				//Delete restored pod
			...
			It("pod with restore resourcePrefix", func() {
				...
				restore := kahu.NewRestoreWithPrefix(restoreName, nsRestore, includeNs, backupName, prefix)
				log.Infof("restore is %v\n", restore)
				restore1, err := kahuClient.KahuV1beta1().Restores().Create(ctx, restore, opts)
				log.Debugf("restore1 is %v\n", restore1)
				Expect(err).To(BeNil())
				...
				//Delete restored pod
				err = k8s.DeletePod(ctx, kubeClient, nsRestore, prefix+name)
				Expect(err).To(BeNil())
				log.Infof(" pod %v in ns %v is deleted\n", prefix+name, nsRestore)
			})
