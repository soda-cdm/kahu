	...
	Describe("deploymentE2ETest", func() {
		Context("Create backup of deployment and restore", func() {
			It("deployment with replicas and pods", func() {
				...
				includeNs := "default"
				backup := kahu.NewBackup(backupName, includeNs)
				opts := metav1.CreateOptions{}
				ctx := context.TODO()
				_, err = kahuClient.KahuV1().Backups().Create(ctx, backup, opts)
				Expect(err).To(BeNil())
				...
				restore1, err := kahuClient.KahuV1().Restores().Create(ctx, restore, opts)
				...
				err = kahuClient.KahuV1().Restores().Delete(ctx, restoreName, optsDel)
				Expect(err).To(BeNil())

				//Delete the backup
				err = kahuClient.KahuV1().Backups().Delete(ctx, backupName, optsDel)
				Expect(err).To(BeNil())
