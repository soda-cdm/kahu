	...
	Describe("configmapE2ETest", func() {
		Context("Create backup of configmap and restore", func() {
			It("configmap", func() {
				...
				log.Infof("configmap is %v\n", configMap)
				Expect(err).To(BeNil())
				err = k8s.WaitForConfigMapComplete(kubeClient, nsRestore, name)
				Expect(err).To(BeNil())
				log.Infof("configmap backedup is up\n")
				...
				err = k8s.WaitForConfigmapDelete(kubeClient, name, ns)

				//Delete restored deployment
				err = k8s.WaitForConfigmapDelete(kubeClient, name, nsRestore)
			})
		})
	})
})
