...
func NewBackupper(logger log.FieldLogger, handler func(key string) error) Interface {
	return &processor{
		logger:  logger,
		handler: handler,
...
func (ctrl *processor) processNextItem() bool {
	...
	if ctrl.queue.NumRequeues(key) < defaultMaxRetry {
		ctrl.logger.WithError(err).WithField("backup", key).Error("Re-adding item to queue")
		ctrl.queue.AddRateLimited(key)
		return true
	}

	ctrl.logger.Infof("Dropping backup (%s) out of the queue. %s", key, err)
	ctrl.queue.Forget(key)
	return true
}
