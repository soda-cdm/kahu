...
type ProviderRegistrationLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.ProviderRegistration, err error)
	// Get retrieves the ProviderRegistration from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.ProviderRegistration, error)
...
// List lists all ProviderRegistrations in the indexer.
func (s *providerRegistrationLister) List(selector labels.Selector) (ret []*v1beta1.ProviderRegistration, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.ProviderRegistration))
	})
	return ret, err
}

// Get retrieves the ProviderRegistration from the index for a given name.
func (s *providerRegistrationLister) Get(name string) (*v1beta1.ProviderRegistration, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("providerregistration"), name)
	}
	return obj.(*v1beta1.ProviderRegistration), nil
}
